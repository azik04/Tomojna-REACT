...
const CreateRate = ({ onClose }) => {
    ...
    const crtRate = async () => {
        try {
            console.log(id);
            console.log(freightForwarderId);
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const res = await axios.get('https://localhost:7161/FreightForwarder/GetAll');
                const freightData = res.data.data || [];
                setFreight(freightData);
                console.log("freight getAll", freightData);
            } catch (error) {
